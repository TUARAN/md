...
- **邮箱账号体系**：新增邮箱验证码注册、邮箱密码登录、会话保持与账号菜单，支持未登录用户在达到使用阈值后引导登录。
- **账号与额度基础设施**：接入 Cloudflare D1、GitHub OAuth、签名 Session、按用户 AI 调用额度，以及 DeepSeek 会话鉴权。
- **内容分发工作流增强**：新增外部 Markdown 导入协议、白名单来源校验、文章导入展示弹窗、掘金签到策略、小红书 campaign 工作流、IP 触达工具、数据准确性闭环和周更新日志页面。
- **SyncBlog Plugin 统一**：统一 SyncBlog Plugin 命名、打包脚本与扩展发布入口，保留旧 `csync` 脚本作为兼容别名，并修复 SegmentFault、百家号等平台编辑器填充链路。
- **创作者与商业化能力**：新增创作者名片分享面板、价格页、设置中心、微信 / 支付宝计费骨架，以及手动 Pro 授权能力。

### 🛠 功能优化与问题修复

- **邮箱注册闭环修复**：修复验证码已通过但注册返回 500 时验证码被提前消费的问题，并降低 Worker 密码哈希成本以避免注册链路超时。
- **GitHub 登录稳定性**：修复 OAuth 回调 Cookie、SPA query 处理和 Worker 路由问题，提升 GitHub 登录成功率。
- **编辑器导航重构**：引入 vue-router，拆分编辑器与分发页面路由，简化顶部导航层级，重做编辑器头部与专注模式。
- **分发体验优化**：合并矩阵与分发视图，优化发布弹窗文案、外部导入去重、平台编辑器打开时的 Markdown 复制逻辑，以及 DeepSeek 改写流程。
- **安全加固**：移除内置图床 Token，要求用户自行配置；使用 AES-GCM 加密图床凭据、用户 API Key 和自定义上传脚本；补充 `innerHTML` 信任链审计文档。
- **Worker 稳定性**：新增 D1 支撑的 IP 级限流、OAuth 启动限流、DeepSeek 聊天限流和 `rate_limits` 夜间清理任务。
- **类型与组件整理**：抽取 `PanelShell`、`SectionHeader`、`Toolbar`、`StatusBadge` 等通用组件，拆分图片上传配置组件，并收敛多处 `any` 类型。

## [v2.2.0] - 2026-05-22

### 🔧 内容分发优化与修复

- **平台注册表统一**：平台 `type` / 名称 / 分类 / 登录入口 / 门户主页收敛到单一注册表（`constants/platforms.ts`），消除发布弹窗、登录链接、名片矩阵、宣发策略多份清单各自漂移的问题；X（原 Twitter）全站统一为 `x`。
- **账号检测健壮性修复**：修复 COSE 检测在「收到部分进度但未回调完成」时检测态永久卡死、「重新检测账号」按钮无法再次使用的问题；超时兜底现无条件复位状态。
- **COSE 发布通道收敛**：发布与账号检测统一走 COSE，避免多扩展桥接对象同时注入造成通道错配。
- **平台矩阵接入检测缓存**：矩阵表的粉丝 / 阅读数据优先取 COSE 检测缓存，缺省回落仓库静态快照。
- **类型与代码清理**：统一扩展桥接对象 `$cose` 的全局类型声明并补全 `CoseApi` 接口，清理冗余分支。
