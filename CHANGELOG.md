...
- **CSYNC / COSE 路由一致性**：发布通道判定收敛为单一规则 `isPluginSyncerAccount`，修复非首选平台「角标 / 进度文案与实际发布通道不符」的错配。
- **平台矩阵接入检测缓存**：矩阵表的粉丝 / 阅读数据优先取 COSE / CSYNC 检测缓存，缺省回落仓库静态快照。
- **类型与代码清理**：统一扩展桥接对象（`$cose` / `$pluginSyncer`）的全局类型声明并补全 `CoseApi` 接口，清理冗余分支。
