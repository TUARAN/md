...
<h1 align="center">AI分发大师</h1>

<p align="center">观点、文章、电子书、项目，一键同步至 73 平台</p>
...
**AI 分发大师** 是面向博主、开发者创作者与内容运营团队的 **AI 内容分发工具**：让 AI 参与你的内容分发，支持**观点、文章、电子书、项目**四类内容，一键同步发布至微信公众号、知乎、微博、掘金沸点、CSDN、掘金等 73 平台。它保留 Markdown 编辑、主题排版、图床与 AI 辅助能力，并强化多平台账号检测、跨平台草稿写入、创作者主页导航展示等分发场景。

其中**多平台分发与同步**子模块以 **Syncblog** 命名：编辑器通过 **Syncblog 发布插件**把同一篇内容写入知乎、公众号、微博、CSDN、掘金等平台草稿；观点线参考 Wechatsync 的“本地浏览器登录态、草稿优先、多平台同步”思路，面向微博、沸点、X、知乎想法等百字短内容场景；同时提供「创作名片」页面，汇总创作者的平台主页，便于对外展示账号矩阵。
...
传统排版工具往往只管「单篇文章美化」，难以承接团队在多平台、多账号下的同步、复用与对外展示。**AI 分发大师** 以「观点、文章、电子书、项目，一键同步至 73 平台」为产品边界：当前版本先把观点验证、写作 / 二创、排版、一键分发（Syncblog 模块）、账号主页展示这条链路跑顺，后续再迭代社群分发、内容校验、互动运营与数据回流。
...
- ✅ **观点分发** - 面向微博、沸点、X、知乎想法等短内容渠道，生成百字左右平台版本
- ✅ **风格二创** - 内置长文提示词模板，可将资讯素材转成可继续排版的 Markdown
- ✅ **内容同步** - 通过 SyncBlog 发布插件，支持多平台草稿写入
...
| 线路     | 说明                                                       | 状态         |
| -------- | ---------------------------------------------------------- | ------------ |
| 观点线   | 观点提炼、短内容改写、微博 / 沸点 / X 等平台版本、互动复盘 | 可用         |
| 文章线   | 素材采集、文章创作、排版发布、矩阵运营、数据闭环           | **核心能力** |
| 电子书线 | 调研选题、内容策划、章节生产、矩阵运营、数据闭环           | 可用         |
| 开源线   | 项目选题、规格策划、工程搭建、矩阵运营、数据闭环           | 可用         |

另提供 **创作名片** 独立页面：汇总各平台已检测到的账号主页，便于对外展示创作者矩阵（入口在编辑器顶栏）。

## 🔌 浏览器扩展（内容同步）

多平台草稿写入依赖 **SyncBlog 发布插件**：

| 插件                                    | 适用场景                                     | 获取方式                                                                  |
| --------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------- |
| [SyncBlog Plugin](/syncblog-plugin.zip) | 知乎 / 公众号 / 微博 / 思否 / 掘金 / CSDN 等 | 从站点下载 `.zip`，`chrome://extensions/` → 「开发者模式」→「加载已解压」 |

> 安装 **SyncBlog Plugin** 后刷新页面，即可在内容同步页检测账号并写入草稿。旧版 COSE 桥接对象仍保留兼容，但后续新能力会优先在 SyncBlog Plugin 中迭代。
