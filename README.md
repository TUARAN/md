...
<h1 align="center">PublishLab</h1>

<p align="center">AI 写作 / 协作 / 出版一站式平台</p>
...
**PublishLab** 是面向博主、开发者创作者与内容运营团队的 **AI 写作 / 协作 / 出版一站式平台**，覆盖从**选题调研 → AI 写作 / 风格二创 → 排版编辑 → 多平台分发 → 商业化与数据复盘**的全链路内容工作台。它保留 Markdown 编辑、主题排版、图床与 AI 辅助能力，并强化多平台账号检测、跨平台草稿写入、创作者主页导航展示等分发场景。

其中**多平台分发与同步**子模块以 **Syncblog** 命名：编辑器通过 **Syncblog 发布插件**把同一篇内容写入知乎、公众号、微博、CSDN、掘金等平台草稿；同时提供「创作名片」页面，汇总创作者的平台主页，便于对外展示账号矩阵。
...
传统排版工具往往只管「单篇文章美化」，难以承接团队在多平台、多账号下的同步、复用与对外展示。**PublishLab** 以「AI 写作 / 协作 / 出版一站式平台」为产品边界：当前版本先把选题、写作 / 二创、排版、分发（Syncblog 模块）、账号主页展示这条链路跑顺，后续再迭代社群分发、内容校验、互动运营与数据回流。
...
| 插件                                                                         | 适用场景                                     | 获取方式                                                                            |
| ---------------------------------------------------------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------- |
| [SyncBlog Plugin](https://github.com/TUARAN/syncblog-plugin/releases/latest) | 知乎 / 公众号 / 微博 / 思否 / 掘金 / CSDN 等 | GitHub Releases 下载 `.zip`，`chrome://extensions/` → 「开发者模式」→「加载已解压」 |
