...
async function startSync() {
  ...
  try {
    if (directPublish.value) {
      try {
        const handledByPlugin = await publishWithExtension()
        if (handledByPlugin)
          return
      }
      catch (e) {
        toast.error(e instanceof Error ? `插件同步失败：${e.message}` : `插件同步失败，内容已复制`)
        return
      }
    }

    toast.success(`内容已复制，正在打开 ${selectedInScope.value.length} 个平台发布页`)
    openSelectedPages(selectedInScope.value)
  }
  finally {
    syncing.value = false
  }
  ...
  <div class="composer-page min-h-0 flex-1 overflow-y-auto">
    <div class="mx-auto w-full max-w-5xl px-4 py-6">
      ...
      <div class="mt-4 flex items-center justify-between gap-3">
        ...
        <button type="button" class="composer-sync" :disabled="syncing" @click="startSync">
          {{ syncing ? '同步中' : '开始同步' }}
          <Send class="size-4" />
        </button>
      </div>
