...
const importExampleCode = computed(() => `// 1. 打开内容同步智能体编辑器
  ...
  <WorkflowPageShell>
    <template #header>
      ...
      <p class="mt-2 text-sm leading-relaxed text-muted-foreground">
        从你的采集站、博客后台、浏览器插件或自动化脚本，把已经生成的 Markdown 直接送入内容同步智能体编辑器。
      </p>
    </template>
      ...
      <section class="rounded-2xl border border-gray-200 bg-white/90 p-5 shadow-sm dark:border-border dark:bg-background">
        ...
        <div class="mt-5 grid gap-3 text-sm">
          <div class="rounded-xl border border-border/70 bg-muted/20 p-3">
            ...
            <p class="mt-1 text-xs leading-relaxed text-muted-foreground">
              外站通过 <code class="rounded bg-muted px-1 py-0.5">window.open()</code> 打开内容同步智能体编辑器，获得可通信的窗口对象。
            </p>
          </div>
          <div class="rounded-xl border border-border/70 bg-muted/20 p-3">
            <p class="font-medium text-foreground">
